...
func CreatetaskbyJSON(c *gin.Context) {
	...
	//https://github.com/gin-gonic/gin/issues/1295
	buf := make([]byte, 1000000)
	num, buferr := c.Request.Body.Read(buf)
	if buferr != nil {
		testlogger.Println("这里从外部请求读入buff发生错误")
		testlogger.Println(buferr)
	}
	reqBody := string(buf[0:num])
	//--------------using gjson to parse------------
	//https://github.com/tidwall/gjson
...
func Update(c *gin.Context) {
	...
	if project != "inbox" {
		db.Model(&task).Update("Project", project)
	}
	if inbox != "nocontent" {
		db.Model(&task).Update("Task", inbox)
	}
	// if parentid_fromgtdcli != "unspecified"{
	...
	if parentproject != "unspecified" {
		db.Model(&task).Update("Parentproject", parentproject)
	}
	fmt.Println("--------iamhere--------")
	fmt.Println("--------iamhere--------")
	fmt.Println("--------iamhere--------")
	...
	if ifdissect != "no" {
		db.Model(&task).Update("Ifdissect", ifdissect)
	}
	if note != "unspecified" {
		db.Model(&task).Update("Note", note)
	}
	if starttime != "unspecified" {

		db.Model(&task).Update("Starttime", starttime)
	}
	if endtime != "unspecified" {
		db.Model(&task).Update("Endtime", endtime)
	}
	if taglight == "yes" {
		db.Model(&task).Update("Tasktags", tasktags)
	}
	if tasktagsorigin != "unspecified" {
		db.Model(&task).Update("Tasktagsorigin", tasktagsorigin)
	}
	// Tasktagsorigin:tasktagsorigin
	if reviewalgolight == "yes" {
		db.Model(&task).Update("Reviewdatas", reviewalgodata)
	}
	if deadline != "unspecified" {
		db.Model(&task).Update("Deadline", deadline)
	}
	if devotedtime != 0 {
		db.Model(&task).Update("Devotedtime", int(devotedtime))
	}
	//EVERYTIME TO COMPUTE
	db.Model(&task).Update("Goalcoefficient", goalcoefficient)
	if isreview == "yes" {
		db.Model(&task).Update("reviewdatas", reviewdata)
	}
...
func Project(c *gin.Context) {
	...
	for _, item := range projects {
		...
		if piot == "no" {
			onlyprojects = append(onlyprojects, item)
		}
	}
